...
const int = Array(+(process.env.NODE_CELL_COUNT || '1000'))
    .fill(0)
    .map(() => lodash_1.random(+(process.env.NODE_CELL_MIN || '0'), +(process.env.NODE_CELL_MAX || Number.MAX_SAFE_INTEGER)));
const big = int.map(BigInt);
exports.suite = new benchmark_1.Suite('number format')
    .add('int.toString(32)', () => lodash_1.shuffle(int).map((i) => i.toString(32)))
    .add('big.toString(32)', () => lodash_1.shuffle(big).map((i) => i.toString(32)))
    .add('int.itos(32)', () => lodash_1.shuffle(int).map((i) => numberFormat_1.itos(i, 32)))
    .add('int.itos(64)', () => lodash_1.shuffle(int).map((i) => numberFormat_1.itos(i, 64)))
    .add('int.itos(62)', () => lodash_1.shuffle(int).map((i) => numberFormat_1.itos(i, 62)))
    .add('Date.now', () => lodash_1.shuffle(int).map(() => Date.now()));
