 ...
 */
export function asyncAction<I extends any[], R, O>(
  executor: (...input: I) => Promise<R>,
  onFulfilled: (result: R) => O,
  onRejected: ((reason: any) => any) | null = null,
  _options: AsyncActionOptions = {},
): AsyncAction<I, O> {
  let func: AsyncAction<I, O>;
  const state = observable({
    status: 'initial' as AsyncStatus,
  ...
  const _onFulfilled = action((result: R) => {
    state.status = 'fulfilled';
    return onFulfilled(result);
  });
  const _onRejected = action((reason: any) => {
  ...
  func = action((...input: I) => {
    ...
    return executor(...input).then(_onFulfilled, _onRejected);
  }) as any;
  Object.assign(func, { state });
  return func;
}
